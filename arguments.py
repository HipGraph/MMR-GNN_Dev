...
class ArgumentParser:
    ...
    def parse_arguments(self, argv, con=None):
        ...
        if not isinstance(argv, list) or len(argv) % 2 != 0:
            raise ValueError(
                "Arguments must be a list (sys.argv) and contain an even number of elements. Received %s of len=%d:\n%s" % (
                    type(argv), len(argv), "\n".join(["%s %s" % (argv[i], argv[i+1]) for i in range(0, len(argv)-1, 2)])
                )
            )
...
class NumPyEncoder(json.JSONEncoder):
    ...
    def default(self, value):
        if isinstance(value, np.integer):
            ...
            return value.tolist()
        return super(NumPyEncoder, self).default(value)
...
class ArgumentBuilder:
    ...
    def build_argvalue(self, value):
        return json.dumps(value, cls=NumPyEncoder).replace(" ", "")
    ...
    def build(self, con):
        ...
        for path, nam_val_par in con.walk():
            ...
            if not isinstance(value, Container) and name not in con.reserved_names:
                argv += self.build_arg(name, value, partition, path)
        return argv
