...
#        plt.axis("off")
    ...
    def _plot_watershed(self, item_shapes_map, subbasin_river_map, path, highlight=True, watershed="", river_opts={"color_code": True, "name": True}):
        ...
        for item, shapes in item_shapes_map.items():
            ...
            for shape_rec in shapes.shapeRecords():
                ...
                x = Transform.minmax_transform(x, min_x, max_x)
                y = Transform.minmax_transform(y, min_y, max_y)
                color = "k"
                if "subbasins" in item:
                    ...
                    subbasin_x = Transform.minmax_transform(subbasin_loc[0], min_x, max_x)
                    subbasin_y = Transform.minmax_transform(subbasin_loc[1], min_y, max_y)
    ...
    def plot_watershed(self, subbasin_shapes, stream_shapes=None, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        subbasin_color = kwargs.get("subbasin_color", "black")
        subbasin_lw = kwargs.get("subbasin_lw", 1)
        stream_color = kwargs.get("subbasin_color", "tab:blue")
        stream_lw = kwargs.get("stream_lw", 1)
        sr_label_idx = kwargs.get("sr_label_idx", None)
        sr_label_xloc_idx = kwargs.get("sr_label_xloc_idx", None)
        sr_label_yloc_idx = kwargs.get("sr_label_yloc_idx", None)
        label_color = kwargs.get("label_color", "yellow")
        ...
        for i, sr in enumerate(subbasin_shapes.shapeRecords()):
            shape, record = sr.shape, sr.record
            label = None if sr_label_idx is None else str(record[sr_label_idx])
            points = np.array(shape.points)
            mean_xy = np.mean(points, 0)
            xmin, xmax = min(np.min(points[:,0]), xmin), max(np.max(points[:,0]), xmax)
            ymin, ymax = min(np.min(points[:,1]), ymin), max(np.max(points[:,1]), ymax)
            self.plot_polygon(points, ax=ax, fill=False, lw=subbasin_lw, color=subbasin_color, zorder=2)
            #
            label_x = None if sr_label_xloc_idx is None else record[sr_label_xloc_idx]
            label_y = None if sr_label_yloc_idx is None else record[sr_label_yloc_idx]
            if label_x is None:
                label_x = mean_xy[0]
            if label_y is None:
                label_y = mean_xy[1]
            if not label is None:
                text = self.plot_text(
                    label_x, label_y, label, ax=ax, fontsize=label_fontsize, color=label_color
                )
                text.set_path_effects([PathEffects.withStroke(linewidth=.1*label_fontsize, foreground="k")])
        if not stream_shapes is None:
            for i, sr in enumerate(stream_shapes.shapeRecords()):
                shape, record = sr.shape, sr.record
                points = np.array(shape.points)[:-1,:]
                self.plot_polygon(points, ax=ax, closed=False, fill=False, color=stream_color, zorder=1)
        self.lim([xmin, xmax], [ymin, ymax], xmargin="0%", ymargin="0%", ax=ax)
        self.ticks([], [], ax=ax)
        ax.axis("off")
        return ax
    ...
    def plot_heatmap(self, mat, xtick_locs=None, xtick_labels=[], ytick_locs=None, ytick_labels=[], xlabel="", ylabel="", cbar_label="", cbar_vmin=None, cbar_vmax=None, cmap="viridis", plot_cbar=True, plot_numbers=False, transpose=False, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        if transpose:
        ...
        im = ax.imshow(mat, cmap=cmap, extent=[-0.5, mat.shape[1]-0.5, -0.5, mat.shape[0]-0.5], **kwargs)
        if plot_cbar:
            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.05)
            vmin, vmax = kwargs.get("vmin", np.min(mat)), kwargs.get("vmax", np.max(mat))
            cbar = plt.colorbar(im, ticks=np.linspace(vmin, vmax, 3), cax=cax)
            cbar.ax.set_ylabel(cbar_label, rotation=-90, va="bottom")
        if xtick_locs is None:
            xtick_locs = np.arange(len(xtick_labels))
        ...
        self.xlabel(xlabel, ax=ax)
        self.ylabel(ylabel, ax=ax)
        return ax
...
#        pdf = pdf / np.sum(pdf)
    ...
    def plot_time_series(self, x, y, temporal_labels=None, mask=None, mask_linestyle="--", ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        line = self.plot_line(x, y, mask, mask_linestyle, ax=ax, **kwargs)
        if not temporal_labels is None:
            index = np.linspace(0, len(temporal_labels)-1, 7, dtype=int)
            self.xticks(index, temporal_labels[index], ax=ax, rotation=60)
        return line

    def plot_line(self, x, y, mask=None, mask_linestyle=None, ax=None, **kwargs):
        if x is None:
            x = range(len(y))
        if ax is None:
            ax = plt.gca()
        ...
        if mask is None:
            ...
            line = ax.plot(x, np.ma.masked_array(y, mask=mask), **kwargs)
            if not mask_linestyle is None:
                _mask = ~mask
                if len(mask) > 1:
                    if mask[0]:
                        _mask[1] = False
                ...
                for i in range(1, len(mask)-1):
                    if mask[i]:
                        _mask[i-1] = False
                        _mask[i+1] = False
                kwargs["color"] = line[-1].get_color()
                kwargs["linestyle"] = mask_linestyle
                kwargs["label"] = None
                line = ax.plot(x, np.ma.masked_array(y, _mask), **kwargs)
        return line

    def get_bar_kwargs(self, **kwargs):
        kwargs = util.merge_dicts(
    ...
    def plot_bar(self, x, y, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        kwargs = self.get_bar_kwargs(**kwargs)
        if x is None:
            x = np.arange(len(y))
    ...
    def plot_multibar(self, x, y, n=1, i=0, ax=None, **kwargs):
        ...
        #
        w = kwargs.pop("width")
        bw = kwargs.pop("bar_width", w / n)
        bb = [-w/2 + bw/2, w/2 - bw/2]
        bx = np.linspace(bb[0], bb[1], n)[i]
        self.plot_bar(x+bx, y, ax=ax, width=bw, **kwargs)
    ...
    def plot_text(self, x, y, text, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        ...
        texts = []
        for _x, _y, _text in zip(x, y, text):
            text = ax.text(_x, _y, _text, **kwargs)
            texts.append(text)
        if len(texts) == 1:
            return texts[0]
        return texts

    def plot_axis(self, loc, axis="x", ax=None, **kwargs):
        ...
        if axis == "x":
            ax.axhline(loc, **kwargs)
        else:
            ax.axvline(loc, **kwargs)

    def plot_rectangle(self, x, y, w, h, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        c = [len(_) if isinstance(_, (tuple, list, np.ndarray)) else 0 for _ in [x, y, w, h]]
        n = max(c)
        if c[0] == 0: # single value x
            x = np.array([x for _ in range(n)])
        if c[1] == 0: # single value y
            y = np.array([y for _ in range(n)])
        if c[2] == 0: # single value w
            w = np.array([w for _ in range(n)])
        if c[3] == 0: # single value h
            h = np.array([h for _ in range(n)])
        x = x - w / 2
        y = y - h / 2
        for i in range(n):
            rec = mpatches.Rectangle((x[i], y[i]), w[i], h[i], **kwargs)
            ax.add_patch(rec)

    def plot_polygon(self, xy, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        poly = mpatches.Polygon(xy, **kwargs)
        ax.add_patch(poly)

    def get_bounds(self, a, b, a_margin=0, b_margin=0):
        _range = b - a
        if isinstance(a_margin, str) and a_margin[-1] == "%":
            a_margin = (float(a_margin[:-1]) / 100) * _range
        if isinstance(b_margin, str) and b_margin[-1] == "%":
            b_margin = (float(b_margin[:-1]) / 100) * _range
        a_new, b_new = a + a_margin, b + b_margin
        if a == round(a) and b == round(b): # integer bounds (e.g. indexes on x-axis for bar plot)
            a_new, b_new = int(a_new), int(b_new)
        return a_new, b_new

    def xlim(self, left=None, right=None, left_margin=0, right_margin=0, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        curr_left, curr_right = ax.get_xlim()
        if not left is None:
            if isinstance(left, str):
                left, _ = self.get_bounds(curr_left, curr_right, a_margin=left)
            else:
                left, _ = self.get_bounds(left, right if right else curr_right, a_margin=left_margin)
            ax.set_xlim(left=left, **kwargs)
        if not right is None:
            if isinstance(right, str):
                _, right = self.get_bounds(curr_left, curr_right, b_margin=right)
            else:
                _, right = self.get_bounds(left if left else curr_left, right, b_margin=right_margin)
            ax.set_xlim(right=right, **kwargs)
        return curr_left, curr_right 

    def ylim(self, bottom=None, top=None, bottom_margin=0, top_margin=0, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        curr_bottom, curr_top = ax.get_ylim()
        if not bottom is None:
            if isinstance(bottom, str):
                bottom, _ = self.get_bounds(curr_bottom, curr_top, a_margin=bottom)
            else:
                bottom, _ = self.get_bounds(bottom, top if top else curr_top, a_margin=bottom_margin)
            ax.set_ylim(bottom=bottom, **kwargs)
        if not top is None:
            if isinstance(top, str):
                _, top = self.get_bounds(curr_bottom, curr_top, b_margin=top)
            else:
                _, top = self.get_bounds(bottom if bottom else curr_bottom, top, b_margin=top_margin)
    ...
    def lim(self, xlim=None, ylim=None, xmargin=0.0, ymargin=0.0, ax=None, xlim_kwargs={}, ylim_kwargs={}):
        def get_margins(margin):
            if isinstance(margin, (int, float, str)): # singleton
                if isinstance(margin, (int, float)): # numeric
                    margin = [-margin, margin]
                else: # str
                    if margin[0] == "-": # pull in xlim
        ...
        if ylim is None:
            ylim = (None, None)
        xmargin = get_margins(xmargin)
        ymargin = get_margins(ymargin)
        #
        left, right = self.xlim(xlim[0], xlim[1], xmargin[0], xmargin[1], ax, **xlim_kwargs)
        bottom, top = self.ylim(ylim[0], ylim[1], ymargin[0], ymargin[1], ax, **ylim_kwargs) 
    ...
    def title(self, label=None, fontdict=None, loc="center", y=None, pad=6.0, ax=None, **kwargs):
        if ax is None:
            ax = plt.gca()
        if label is None:
            return ax.get_title()
        return ax.set_title(label, fontdict=fontdict, loc=loc, y=y, pad=pad, **kwargs)
...
#        kwargs = util.merge_dicts({"size": 7}, kwargs)
    ...
    def subplots(self, nrow=1, ncol=1, size=(8, 8)):
        return plt.subplots(nrow, ncol, figsize=size)

    def save_figure(self, path, fig=None, dpi=200):
        if fig is None:
            fig = plt
    ...
    def plot_learning_curve(self, train, valid, test, path):
        ...
        bottom, top = plt.ylim()
        try: # ValueError when ymax is inf/nan from model failure
...
#        plt.yticks(ytick_indices, ytick_labels, fontsize=6)
    ...
    def plot_metric_scatter(self, model_i_metrics, model_j_metrics, datasets, var, ax=None):
        ...
        def get_node_metrics(model_metrics, var):
            return np.array(list(model_metrics.get(var.metric, var.partition).get(var.response_feature).values()))
        def get_node_means(datasets, var):
            dataset = datasets.get("dataset", "train")
            return dataset.spatiotemporal.filter_axis(
                dataset.spatiotemporal.statistics.means, 
                [0, 1], 
                [
        ...
        if ax is None:
            ax = plt.gca()
        # Get spatial resolution metrics
        points_legend_label = datasets.get("dataset", var.partition).spatiotemporal.misc.spatial_label_field
        spatial_metric_map_i = model_i_metrics.get(var.metric, var.partition).get(var.response_feature)
        spatial_metric_map_j = model_j_metrics.get(var.metric, var.partition).get(var.response_feature)
        spatial_labels_i, metrics_i = list(spatial_metric_map_i.keys()), np.array(list(spatial_metric_map_i.values()))
        spatial_labels_j, metrics_j = list(spatial_metric_map_j.keys()), np.array(list(spatial_metric_map_j.values()))
        if 0:
            print("model_i_metrics =", spatial_metric_map_i)
            print("model_j_metrics =", spatial_metric_map_j)
        ...
        graph = dataset.graph
        spatial_labels = spatiotemporal.original.get("spatial_labels", var.partition)
        #
        if var.plot_args.x_axis == "in-degree":
            ...
            x_i = get_node_metrics(model_i_metrics, var)
            x_j = get_node_metrics(model_j_metrics, var)
        ...
        if var.plot_args.z_axis is None:
            ...
            z_i = get_node_metrics(model_i_metrics, var)
            z_j = get_node_metrics(model_j_metrics, var)
            xlabel = "$%s_{%s}$" % (var.metric, var.model_i)
        elif var.plot_args.z_axis == "mean":
            z_i = get_node_means(datasets, var)
            z_j = get_node_means(datasets, var)
            ...
            z_i, z_j = np.squeeze(z_i), np.squeeze(z_j)
        elif var.plot_args.z_axis == "cov": # coefficient of variation (s / u)
            z_i = get_node_stds(datasets, var) / get_node_means(datasets, var)
            z_j = get_node_stds(datasets, var) / get_node_means(datasets, var)
            z_i, z_j = np.squeeze(z_i), np.squeeze(z_j)
        else:
            raise NotImplementedError("var.plot_args.z_axis=%s" % (var.plot_args.z_axis))
        if "diff" in var.plot_args.y_axis:
            x = x_i
            y = metrics_j - metrics_i
            if "%" in var.plot_args.y_axis:
                y = ((y / metrics_i) * 100).round(2)
            index = np.argsort(y)
            print(metrics_i[index[:10]])
            print(metrics_j[index[:10]])
            print(y[index[:10]])
            print(spatial_labels[index[:10]])
            z = z_i
            if var.plot_args.z_axis is None:
                z_alpha = z
                z_scale = z * 75
                ...
                z_alpha = Transform.minmax_transform(z, min(z), max(z), a=1/10, b=1)
                z_scale = Transform.minmax_transform(z, min(z), max(z), a=10, b=100)
                z_alpha = np.ones(z.shape)
                z_color = [
                    plt.get_cmap("Blues")(fac) for fac in Transform.minmax_transform(z, min(z), max(z), a=3/8, b=1)
                ]
            self.plot_axis(0, "x", ax=ax, color="k", linestyle="-", alpha=4/8, zorder=0)
            ...
            if "trend" in var.plot_args.plot_options:
                self.plot_trend(x, y, ax=ax, label="Trend")
            ylabel = "$\Delta$ %s (%s $\longrightarrow$ %s)" % (var.metric, var.model_i, var.model_j)
            if "%" in var.plot_args.y_axis:
                ylabel = "%% %s" % (ylabel)
            self.ylabel(ylabel, ax=ax)
        elif var.plot_args.y_axis == "raw":
            y_i, y_j = metrics_i, erorrs_j
            self.plot_scatter(x_i, y_i, ax=ax, scatter_kwargs={"c": "r", "label": var.model_i})
            if "trend" in var.plot_args.plot_options:
                self.plot_trend(x, y, ax=ax, color="r", label="%s Trend" % (var.model_i))
            x, y = indegrees, metrics_j
            self.plot_scatter(x_j, y_j, ax=ax, scatter_kwargs={"c": "b", "label": var.model_j})
            if "trend" in var.plot_args.plot_options:
                self.plot_trend(x, y, ax=ax, color="b", label="%s Trend" % (var.model_j))
            self.ylabel(var.metric, ax=ax)
        else:
            raise NotImplementedError("var.plot_args.y_axis=%s" % (var.plot_args.y_axis))
        if var.plot_args.x_axis == "in-degree":
            self.xticks(None, np.arange(0, max(x)+1), ax=ax)
            self.xlim(-0.5, max(x)+0.5, ax=ax)
        self.xlabel(xlabel, ax=ax)
        self.legend(ax=ax)
        return ax
