...
class Experiment:
    ...
    def __init__(self):
        ...
        grid_args = Container()
        grid_args = self.add_first_args(grid_args)
...
def get_exp(module, exp_id=None):
    if exp_id is None:
        return get_user_exp(module)
    return getattr(module, "Experiment__%s" % (str(exp_id).replace(".", "_")))()


def get_user_exp(module):
    exp_id = input("Experiment: ")
    print()
    return get_exp(module, exp_id)
