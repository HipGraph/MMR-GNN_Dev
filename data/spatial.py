    ...
    def transform_categorical(self, features, feature_labels, feature_catlabels_map={}):
        ...
        for i, feature_label in enumerate(feature_labels):
            if feature_label in feature_catlabels_map:
                encoder = transform.LabelEncoder().fit(feature_catlabels_map[feature_label])
            else:
                encoder = transform.LabelEncoder().fit(features[:,i])
            feature_onehots_map[feature_label] = encoder.transform(features[:,i], sparse=False)
