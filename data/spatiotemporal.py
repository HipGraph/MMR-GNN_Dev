...
class SpatiotemporalData(Container, DataSelection):
    ...
    def __init__(self, var, verbose=0):
...
class Original(Container, DataSelection):
    ...
    def __init__(self, var):
        ...
        self.period_size = np.max(self.periodic_indices) + 1
        #   Feature data
    ...
    def transform(self, features, periodic_indices, spatial_indices, feature_indices, var, revert=False):
        ...
        if transform_resolution == ["temporal", "spatial", "feature"]:
            for j, (feature_label, f) in enumerate(zip(feature_labels, feature_indices)):
                _transform = feature_transform_map[feature_label]
                if isinstance(_transform, dict): _transform = _transform["name"]
                features[:,:,j] = transform.transform(
                    [
                        features[:,:,j],
                        transform_args_map[_transform][0][periodic_indices,:,:][:,spatial_indices,f],
                        transform_args_map[_transform][1][periodic_indices,:,:][:,spatial_indices,f]
                    ], 
            ...
            for j, (feature_label, f) in enumerate(zip(feature_labels, feature_indices)):
                ...
                features[:,:,j] = transform.transform(
                    [
                        features[:,:,j],
                        transform_args_map[_transform][0][periodic_indices,f], 
                        transform_args_map[_transform][1][periodic_indices,f]
                    ], 
                    feature_transform_map[feature_label], 
                    revert
                )
            ...
            for j, (feature_label, f) in enumerate(zip(feature_labels, feature_indices)):
                ...
                features[:,:,j] = transform.transform(
                    [
                        features[:,:,j], 
                        transform_args_map[_transform][0][spatial_indices,f], 
                        transform_args_map[_transform][1][spatial_indices,f]
                    ],
                    feature_transform_map[feature_label], 
                    revert
                )
            ...
            for j, (feature_label, f) in enumerate(zip(feature_labels, feature_indices)):
                ...
                features[:,:,j] = transform.transform(
                    [
                        features[:,:,j], 
                        transform_args_map[_transform][0][f], 
                        transform_args_map[_transform][1][f]
                    ], 
                    feature_transform_map[feature_label], 
                    revert
                )
...
class Reduced(Container, DataSelection):
    ...
    def __init__(self, var):
        ...
        self.gtmask = self.reduce_gtmask(orig_var.gtmask, ["or"]+proc_var.temporal_reduction[1:], tmp_var)
        # Filter/init data for each partition
        for partition in part_var.partitions:
            #   Spatial label data
    ...
    def reduce_gtmask(self, gtmask, temporal_reduction, var):
        n_temporal, n_spatial, n_feature = gtmask.shape
        method, factor, stride = temporal_reduction
        method_fn_map = {
            "or": np.any, 
            "and": np.all, 
        ...
        indices = self.get_temporal_reduction_indices(n_temporal, temporal_reduction)
        reduced_gtmask = method_fn_map[method](gtmask[indices,:,:], 2)
        return reduced_gtmask
    ...
    def get_approximate_temporal_index(self, target_temporal_label, temporal_labels, temporal_resolution, temporal_label_format):
        ...
        while end > start:
            mid = (start + end) // 2
            start_rpos = self.get_relative_temporal_position(
                temporal_labels[start], 
                target_temporal_label,
                temporal_resolution, 
                temporal_label_format
            )
            end_rpos = self.get_relative_temporal_position(
                temporal_labels[end], 
                target_temporal_label,
                temporal_resolution, 
                temporal_label_format
            )
            mid_rpos = self.get_relative_temporal_position(
                temporal_labels[mid], 
                target_temporal_label,
                temporal_resolution, 
                temporal_label_format
            )
            start_dist, mid_dist, end_dist = abs(start_rpos), abs(mid_rpos), abs(end_rpos)
            if 0:
                print(start, mid, end)
                print(target_temporal_label, ":", temporal_labels[start], temporal_labels[mid], temporal_labels[end])
                print(start_rpos, mid_rpos, end_rpos)
                print(start_dist, mid_dist, end_dist)
                input()
            if start_dist <= end_dist: # go left
                end = mid
            else: # go right
                start = mid + 1
        start_rpos = self.get_relative_temporal_position(
        ...
        if start_rpos > 0: # closest time-step on right, move it to first on left
            start = max(0, start - 1) # limit to start in [0,?]
        return start

    def get_relative_temporal_position(self, a, b, temporal_resolution, temporal_label_format):
        if a == "" or b == "":
...
class ReducedStatistics(Container, DataSelection):
    ...
    def __init__(self, var):
        ...
        # Load/init statistics
        def compute_statistic(features, stat_fn):
            return stat_fn(features, (0, 1))
        features = red_var.filter_axes(red_var.features,[0, 1], red_var.get("temporal_indices", "train"))
        self.minimums = compute_statistic(features, np.min)
        self.maximums = compute_statistic(features, np.max)
        self.medians = compute_statistic(features, np.median)
        self.means = compute_statistic(features, np.mean)
...
class Windowed(Container, DataSelection):
        ...
        if "temporal" in transform_resolution and "spatial" in transform_resolution:
            for i in range(n_window):
                ...
                for j, (feature_label, f) in enumerate(zip(feature_labels, feature_indices)):
                    _transform = feature_transform_map[feature_label]
                    if isinstance(_transform, dict): _transform = _transform["name"]
                    features[i,:,:,j] = transform.transform(
                        [
                            features[i,:,:,j],
                            transform_args_map[_transform][0][periodic_indices,:,:][:,spatial_indices,f],
                            transform_args_map[_transform][1][periodic_indices,:,:][:,spatial_indices,f]
    ...
    def transform_reduced(self, features, periodic_indices, spatial_indices, feature_indices, var, revert=False):
        # Unpack vars
        n_channel, n_window, n_temporal, n_spatial, n_feature = features.shape
        feature_labels = util.get_dict_values(var.index_feature_map, feature_indices)
        transform_resolution = var.spatiotemporal.transform_resolution
        feature_transform_map = var.spatiotemporal.feature_transform_map
        default_feature_transform = var.spatiotemporal.default_feature_transform
...
def compute_histograms(features, bins=100, lims=None):
    n_temporal, n_spatial, n_feature = features.shape
    hists = np.concatenate(
        [Probability.compute_histograms(features[:,:,i], bins=bins, lims=lims) for i in range(n_feature)], -1
    )
    return hists
...
def compute_correlations(features):
    n_temporal, n_spatial, n_feature = features.shape
    corrs = Probability.compute_correlations(np.reshape(features, (n_temporal, -1)))
    return np.reshape(corrs, (n_spatial, n_feature, n_spatial, n_feature))


def compute_representation(spatmp, rep="histogram", partition="train", **kwargs):
    # Handle arguments
    bins, lims = kwargs.get("bins", 100), kwargs.get("lims", None)
    features = kwargs.get("features", "response")
...
#        x = compute_histograms(spatmp.transformed.original.get("predictor_features", partition), bins, lims)
        x = compute_histograms(
            spatmp.transformed.original.get("%s_features" % (features), partition), bins, lims
        )
        if debug:
            print("x =", x.shape)
            if debug > 1:
        ...
        x = compute_correlations(spatmp.transformed.original.get("%s_features" % (features), partition))
        if len(spatmp.misc.response_features) > 1:
            raise ValueError(
                "Must select one response feature for correlation. Received response_features=%s" % (
                    spatmp.misc.response_features
                )
            )
        idx = spatmp.misc.get("%s_features" % (features)).index(spatmp.misc.response_features[0])
        if debug > 2:
            print("x =", x.shape)
            if debug > 3:
                print(spatmp.misc.predictor_features)
                x = np.round(x, 3)
                print(x[0,...,:20])
                print(x[1,...,:20])
        if 0:
            x = np.stack([x[i,idx,i,:] for i in range(x.shape[-2])], 0)
        else:
            x = x[:,idx,:,:]
        x = np.reshape(x, (x.shape[0], -1))
        if debug:
...
def cluster(spatmp, alg="Agglomerative", n_clusters=3, rep="histogram", partition="train", **kwargs):
    ...
    x = compute_representation(spatmp, rep, partition=partition, **kwargs)
    #  Compute clusters
    if alg in ["Agglomerative"] and rep in ["Wasserstein"]:
        clustering, cluster_index = Clustering.cluster(x, alg, n_clusters, return_obj=1, affinity="precomputed")
    else:
        clustering, cluster_index = Clustering.cluster(x, alg, n_clusters, return_obj=1)
    return x, clustering, cluster_index


def treeify(spatmp, alg="Agglomerative", rep="histogram", partition="train", **kwargs):
    # Handle arguments
    debug = kwargs.get("debug", 0)
    # Start
    #  Derive representation of data x
    ...
    #  Compute clusters
    if alg in ["Agglomerative"] and rep in ["Wasserstein"]:
        clustering, cluster_index = Clustering.cluster(x, alg, 1, return_obj=1, affinity="precomputed")
    else:
        clustering, cluster_index = Clustering.cluster(x, alg, 1, return_obj=1)
    #  Convert to tree
    if alg == "Agglomerative":
       tree_index = clustering.children_
    else:
        raise NotImplementedError(alg)
    return x, clustering, tree_index


def contiguify(features, temporal_mapping, horizon, stride=1, offset=0):
    N, T = features.shape[:2]
    indices = util.contiguous_window_indices(N, T, stride, offset)
    return np.take(features, indices, 0).reshape((-1,)+features.shape[2:])
