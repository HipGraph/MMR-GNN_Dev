...
from model.ASTGCN_PyTorch.model.ASTGCN_r import make_model
...
class ASTGCN(Model):
    ...
    def reset_parameters(self):
        # Acquired from function make_model() @ model/ASTGCN_r.py
    ...
    def verify_pulled_data(self, data, var):
        if data["y"].shape[-1] > 1:
            raise ValueError(
                "ASTGCN can only infer a single response variable but \"y\" has shape=%s" % (data["y"].shape)
            )
        return data
